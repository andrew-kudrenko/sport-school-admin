...
export const CoachesEditorLayout: React.FC<IEntityEditorProps> = ({
  mode,
  title,
}) => {
  ...
  let { value: coach, loading: fetching } = useGetQuery<
                ...
                {(selected as string[])?.length
                  ? (selected as string[]).map((value) => (
                      <Chip
                        ...
                        label={`${
                          groups.find((g) => g.id === value)?.year || ""
                        } ${
                          schools.find(
                            (s) =>
                              s.id ===
                              groups.find((g) => g.id === value)?.school_id
                          )?.name || ""
                        }`}
                      />
                    ))
          ...
          >
            ...
            {groups.map((g) => (
              <MenuItem value={g.id} key={g.id}>
                {`${g.year} ${
                  schools.find((s) => s.id === g.school_id)?.name || ""
                }`}
              </MenuItem>
            ))}
          </Select>
